...
export function DiffViewer({ tab, workspace }: DiffViewerProps) {
  ...
  const diffOptions: FileDiffOptions<undefined> = {
    theme: "pierre-dark",
    themeType: "dark",
    diffStyle: settings.diffInline ? "unified" : "split",
    diffIndicators: "bars",
  ...
  return (
    <section className="diff-panel">
      <div className="surface-toolbar">
        <div className="toolbar-title">{tab.activePath ? `Changes: ${tab.activePath}` : "Workspace Changes"}</div>
        <div className="toolbar-actions">
          <label className="switch-row compact-switch">
            <input
              type="checkbox"
              checked={settings.diffInline}
              onChange={(event) => updateSettings({ diffInline: event.currentTarget.checked })}
            />
            Unified
          </label>
      ...
      {!loading && statuses.length === 0 ? <div className="panel-placeholder">No git changes</div> : null}
      {!loading && diffs.length === 0 && statuses.length > 0 ? (
        <div className="panel-placeholder">Select a changed file from the Changes panel</div>
      ) : null}
      <div className="diff-scroll">
        {diffs.map((file) => {
          const selectedStatus = statuses.find((status) => keyForStatus(status) === `${file.bucket}:${file.path}`);
            ...
            <article className="diff-file" key={`${file.bucket}:${file.path}`}>
              <header className="diff-file-header">
                ...
                <button
                  ...
                  type="button"
                  onClick={() =>
                    addContextDiff(workspace.id, file.path, file.bucket, selectedStatus?.status ?? file.status)
                  }
                >
                  Add Diff
                </button>
              ...
              {file.isBinary ? (
                ...
                <PatchDiff patch={file.patch} options={options} disableWorkerPool />
              ) : (
              ...
              {pending?.file.path === file.path && pending.file.bucket === file.bucket ? (
                <div className="inline-comment-box">
                  ...
                  <textarea
                    value={pending.text}
                    onChange={(event) => setPending({ ...pending, text: event.currentTarget.value })}
