import { FileCode2, GitCompare, MessageSquare, SendHorizontal, Trash2, X } from "lucide-react";
...
export function ContextPanel() {
  ...
  const items = useAppStore((state) =>
    state.contextItems.filter((item) => item.workspaceId === state.activeWorkspaceId),
  );
  ...
  return (
    <section className="panel-body context-panel">
      ...
      <div className="context-list">
        {items.length === 0 ? (
          <div className="panel-placeholder slim">Select files, diffs, or diff ranges to build context</div>
        ) : (
          items.map((item) => (
      ...
      <button className="secondary-button full danger-text" type="button" disabled={items.length === 0} onClick={() => clearWorkspaceContext(workspace.id)}>
        <Trash2 size={15} />
        Clear Context
      </button>
    </section>
