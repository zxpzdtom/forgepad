...
function ArgValue({ arg }: { arg: ConsoleArg }) {
  ...
  if (arg.type === "symbol") {
    return (
      <span className="text-[#c586c0]">{arg.description ?? "Symbol()"}</span>
    );
  }
  if (arg.type === "function") {
    return (
      <span className="text-subtle italic">{arg.description ?? "f()"}</span>
    );
  }
  if (arg.type === "object") {
    if (arg.subtype === "null")
      return <span className="text-subtle">null</span>;
    if (arg.subtype === "regexp")
      return <span className="text-[#d16969]">{arg.description}</span>;
    if (arg.subtype === "date")
      return <span className="text-[#ce9178]">{arg.description}</span>;
    ...
    if (arg.preview?.properties) {
      const isArray =
        arg.subtype === "array" || arg.preview.subtype === "array";
      const open = isArray ? "[" : "{";
      const close = isArray ? "]" : "}";
      return (
        <span className="text-text">
          {arg.className && !isArray && (
            <span className="mr-1 text-[#4ec9b0]">{arg.className}</span>
          )}
...
function LevelBadge({ level }: { level: ConsoleEntry["level"] }) {
  ...
  return (
    <span
      className={`inline-flex w-[28px] shrink-0 justify-center font-mono text-[10px] font-semibold ${color}`}
    >
      {labels[level]}
    </span>
  );
...
export function BrowserConsolePanel({
  ...
  const filteredEntries = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter((e) =>
        stringifyConsoleArgs(e.args).toLowerCase().includes(q),
      );
    }
    return result;
  ...
  const toggleSelectAll = useCallback(() => {
    setSelectedIds((prev) => {
      ...
      const allSelected =
        allFilteredIds.length > 0 && allFilteredIds.every((id) => prev.has(id));
      if (allSelected) {
        const next = new Set(prev);
  ...
  return (
    <section className="flex size-full min-h-0 flex-col bg-panel">
      ...
      <div className="surface-toolbar flex h-9 shrink-0 items-center gap-1.5 border-border border-b px-2">
        {/* Title */}
        <span className="mr-0.5 font-medium text-muted text-[13px] select-none">
          ...
          {(["all", "error", "warn", "log", "debug"] as const).map((level) => {
            ...
            return (
              ...
              >
                ...
                {count > 0 && level !== "all" && (
                  <span
                    className={`ml-1 font-mono text-[10px] ${isActive ? levelColor || "text-subtle" : "opacity-50"}`}
                  >
                    {count}
                  </span>
                )}
        ...
        {!hideSendToAgent && filteredEntries.length > 0 && (
          ...
          >
            ...
            <span className="select-none">
              {allFilteredSelected
                ? t("browserConsole.deselect")
                : t("browserConsole.selectAll")}
            </span>
          </span>
        )}
        ...
        {!hideSendToAgent && (
          ...
          >
            ...
            {selectedIds.size > 0
              ? t("browserConsole.sendCount", { count: selectedIds.size })
              : t("common.send")}
          </span>
        )}
            ...
            return (
              ...
              >
                  ...
                  <div
                    className={[
                      ...
                      isSelected
                        ...
                        : "border-subtle/40 group-hover:border-muted",
                    ].join(" ")}
                  >
                    <svg
                      width="8"
                ...
                {isInput ? (
                  ...
                  <span className="inline-flex w-[28px] shrink-0 justify-center font-mono text-[13px] text-subtle">
                    ‹
                  </span>
                ) : (
