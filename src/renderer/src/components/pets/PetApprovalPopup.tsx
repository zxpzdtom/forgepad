...
export function PetApprovalPopup({
  ...
  const fileName = (() => {
    const fp =
      permission.toolInput?.file_path ?? permission.toolInput?.filePath;
    if (typeof fp === "string") {
...
function OptionButton({
  ...
  const bgNormal = selected
    ? "rgba(60, 100, 180, 0.5)"
    : "rgba(60, 80, 140, 0.3)";
  const bgHover = selected
    ? "rgba(70, 110, 200, 0.6)"
    : "rgba(80, 110, 180, 0.45)";
  const borderNormal = selected
    ? "rgba(100, 150, 240, 0.6)"
    : "rgba(80, 110, 180, 0.3)";
  const borderHover = selected
    ? "rgba(120, 170, 255, 0.7)"
    : "rgba(100, 140, 220, 0.5)";
