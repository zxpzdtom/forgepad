...
function ExtensionRow({ ext, onUninstall }: { ext: ExtensionInfo; onUninstall: (ext: ExtensionInfo) => void }) {
    ...
    <div className="group flex items-center gap-3 rounded-lg border border-border bg-panel p-3 transition-colors hover:bg-panel-2">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <span className="truncate font-[510] text-[13px] text-text">{ext.name}</span>
          <span className="shrink-0 rounded bg-border/50 px-1.5 py-0.5 text-[10px] text-subtle">v{ext.version}</span>
        </div>
        <div className="mt-0.5 truncate text-[11px] text-subtle">{ext.path}</div>
      </div>
...
export function ExtensionsSection() {
  ...
  return (
    <div>
      ...
      <p className="mb-4 text-[12px] leading-relaxed text-subtle">{t('settings.extensions.description')}</p>

      {/* Error banner */}
      {error && (
        <div className="mb-4 rounded-lg border border-red-500/30 bg-red-500/10 px-3 py-2 text-[12px] text-red-400">{error}</div>
      )}
      ...
      >
        <FolderOpen size={15} />
        {loading ? t('settings.extensions.installing') : t('settings.extensions.addExtension')}
      </button>
      ...
      {extensions.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p className="text-[13px] text-subtle">{t('settings.extensions.empty')}</p>
          <p className="mt-1 text-[11px] text-subtle/60">{t('settings.extensions.emptyHint')}</p>
        </div>
      ) : (
        <div className="flex flex-col gap-2">
          {extensions.map((ext) => (
            <ExtensionRow key={ext.id} ext={ext} onUninstall={handleUninstall} />
          ))}
        </div>
      )}
    </div>
