...
export function useAgentLifecycle(): void {
  ...
  useEffect(() => {
    ...
    const triggerNotification = async (ptyId: string, status: AgentStatus) => {
      ...
      const shouldSound =
        ns.enabled &&
        ((isAgentDone && ns.notifyOnAgentDone) ||
          (isNeedsApproval && ns.notifyOnAgentNeedsApproval));
      ...
      if (shouldDesktop && !appFocused) {
        ...
        const body = isAgentDone
          ? `${tabTitle} has finished its task.`
          : `${tabTitle} is waiting for your approval.`;
    ...
    const removePermissionListener = window.forgepad.agent.onPermissionRequest(
      (data) => {
    ...
    // Listen for user prompt submissions (full prompt for completion card)
    const removeUserPromptListener = window.forgepad.agent.onUserPrompt(
      (data) => {
        useAppStore.getState().setAgentUserPrompt(data.ptyId, data.prompt);
      },
    );

    // Listen for agent completion events (AI's last response)
    const removeCompletionListener = window.forgepad.agent.onCompletion(
      (data) => {
        useAppStore.getState().setAgentCompletion(data.ptyId, data.aiMessage);
      },
    );
...
function playNotificationSound(
  ...
  // Try built-in sound
  const builtin =
    BUILTIN_SOUNDS.find((s) => s.id === ns.selectedSoundId) ??
    BUILTIN_SOUNDS[0];
  if (!builtin) return;
