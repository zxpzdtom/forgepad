...
async function safeReadText(rootPath: string, relPathInput: string): Promise<{ text: string; truncated: boolean } | null> {
  ...
  if (raw.length > MAX_CONTEXT_FILE_CHARS) {
    return {
      text: `${raw.slice(0, MAX_CONTEXT_FILE_CHARS)}\n\n[Truncated for context budget]`,
      truncated: true,
    };
  }
  ...
  static async createBundle(input: CreateBundleInput): Promise<ContextBundleResult> {
    ...
    if (input.comments.length > 0) {
      ...
      for (const comment of input.comments) {
        const range =
          comment.startLine === comment.endLine ? `L${comment.startLine}` : `L${comment.startLine}-L${comment.endLine}`;
        lines.push(`- ${comment.relPath} ${range} ${comment.side}: ${comment.text.trim()}`);
      }
      lines.push('');
    }
    ...
    if (input.selections.length > 0) {
      ...
      for (const selection of input.selections) {
        ...
        const range =
          selection.startLine === selection.endLine ? `L${selection.startLine}` : `L${selection.startLine}-L${selection.endLine}`;
        lines.push(`### ${relPath} ${range}`, '');
        lines.push(`User note: ${selection.text.trim()}`, '');
        lines.push(`\`\`\`${fenceForPath(relPath)}`, selection.selectedText.slice(0, 20_000), '```', '');
      }
    }
