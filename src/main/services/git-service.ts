...
export class GitService {
   ...
   * - Alibaba Code (code.alibaba-inc.com) is GitLab-based, uses refs/merge-requests/<n>/head
   *   but has a different web URL pattern: /codereview/<n> instead of /-/merge_requests/<n>
   *
   ...
   *
   * Merge detection:
   * - We also check if the MR/PR has been merged into a real target branch
  ...
  static async getPrInfo(
    ...
    try {
      ...
      const isAlibabaCode = /\.alibaba-inc\.com$/i.test(parsed.host);
      ...
      const buildUrl = (prNum: number): string => {
        ...
        if (isGitLab) return `${baseUrl}/-/merge_requests/${prNum}`;
      ...
      // No MR merged to master/main — return the first matched MR as open/unmerged
      const firstPr = matchedPrNums[0];
      return { number: firstPr, url: buildUrl(firstPr), merged: false };
    } catch {
      return null;
    }
  }
  ...
  /**
   * Check whether a PR/MR has been merged into a real main branch (master/main).
   *
   * WAP deploy branches (e.g. wap_release_2026_04_23_16_34_54_test/1.2.0-test)
   * are NOT considered real merges — only master and main count.
   *
   * Detection approach:
    ...
    try {
      ...
      for (const mainBranch of ["origin/master", "origin/main"]) {
        try {
          ...
          // exit 1 = not an ancestor, or branch doesn't exist — try next
        }
      }
