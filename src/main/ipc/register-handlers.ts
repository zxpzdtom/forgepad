...
import { app, BrowserWindow, dialog, ipcMain, screen, session, shell, webContents } from 'electron';
...
async function openInIde(fullPath: string): Promise<void> {
  const commands =
    process.env.FORGEPAD_IDE_COMMAND?.split(',')
      .map((item) => item.trim())
      .filter(Boolean) ?? [];
  const candidates = commands.length > 0 ? commands : ['code', 'cursor', 'windsurf', 'zed'];
...
async function openWithTerminal(fullPath: string, terminalId: string): Promise<void> {
  if (process.platform === 'darwin') {
...
export function registerIpcHandlers(hookPort?: number, hookServer?: import('../services/hook-server').HookServer): void {
  ...
  ipcMain.handle(IPC.GIT_CURRENT_BRANCH, async (_event, worktreePath: string) => GitService.getCurrentBranch(worktreePath));
  ipcMain.handle(IPC.GIT_BRANCH_STATS, async (_event, worktreePath: string) => GitService.getBranchStats(worktreePath));
  ipcMain.handle(IPC.GIT_STATUS, async (_event, worktreePath: string) => GitService.getStatus(worktreePath));
  ipcMain.handle(IPC.GIT_FILE_DIFF, async (_event, worktreePath: string, relPath: string, bucket, status, oldPath?: string) =>
    GitService.getFileDiff(worktreePath, relPath, bucket, status, oldPath),
  );
  ipcMain.handle(IPC.GIT_STAGE, async (_event, worktreePath: string, paths: string[]) => GitService.stage(worktreePath, paths));
  ipcMain.handle(IPC.GIT_UNSTAGE, async (_event, worktreePath: string, paths: string[]) =>
    GitService.unstage(worktreePath, paths),
  );
  ipcMain.handle(IPC.GIT_DISCARD, async (_event, worktreePath: string, entries) => GitService.discard(worktreePath, entries));
  ipcMain.handle(IPC.GIT_COMMIT, async (_event, worktreePath: string, message: string) =>
    GitService.commit(worktreePath, message),
  );
  ipcMain.handle(IPC.GIT_PUSH, async (_event, worktreePath: string) => GitService.push(worktreePath));
  ipcMain.handle(IPC.GIT_PULL, async (_event, worktreePath: string) => GitService.pull(worktreePath));
  ipcMain.handle(IPC.GIT_GENERATE_COMMIT_MSG, async (_event, worktreePath: string, promptTemplate: string) =>
  ...
  ipcMain.on(IPC.PTY_RESIZE, (_event, id: string, cols: number, rows: number) => ptyService.resize(id, cols, rows));
  ...
  ipcMain.handle(IPC.CONTEXT_CREATE_BUNDLE, async (_event, input: CreateBundleInput) => ContextService.createBundle(input));
  ...
  ipcMain.handle(IPC.BROWSER_OPEN_DEVTOOLS, async (_event, args: { webContentsId: number }) => {
    const wc = webContents.fromId(args.webContentsId);
    if (!wc || wc.isDestroyed()) return;
   ...
   */
  async function resolveExtensionIconDataUrl(ext: Electron.Extension): Promise<string | null> {
  ...
  ipcMain.handle(IPC.EXTENSION_LIST, async () => {
    return Promise.all(session.defaultSession.getAllExtensions().map(toExtensionInfo));
  });
  ...
  ipcMain.handle(
    ...
    async (_event, args: { tabId: number; func?: string; files?: string[]; extId?: string }) => {
      ...
      if (args.files && args.extId) {
        const ext = session.defaultSession.getAllExtensions().find((e) => e.id === args.extId);
  ...
  ipcMain.handle(IPC.EXTENSION_SCRIPTING_INSERT_CSS, async (_event, args: { tabId: number; css: string }) => {
    const wc = webContents.fromId(args.tabId);
    if (!wc || wc.isDestroyed()) return;
  ...
  const pendingTabCreates = new Map<string, { resolve: (tabId: number) => void; timer: NodeJS.Timeout }>();

  ipcMain.handle(IPC.EXTENSION_TAB_CREATE, async (_event, args: { url: string; active?: boolean }) => {
  ...
  ipcMain.on(IPC.EXTENSION_TAB_CREATED, (_event, args: { requestId: string; webContentsId: number }) => {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      extensionPopupSenderWin = BrowserWindow.fromWebContents(_event.sender) ?? null;
  ...
  if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.local && !chrome.storage.sync) {
    chrome.storage.sync = chrome.storage.local;
        ...
        console.warn('[ForgePad] Failed to inject extension polyfills via CDP:', err);
      }

      extensionPopup.loadURL(`chrome-extension://${args.extId}/${args.popupPath}`);
