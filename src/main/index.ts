...
/** Restore the main application menu (called when main window regains focus). */
export function restoreAppMenu(): void {
  if (mainAppMenu) Menu.setApplicationMenu(mainAppMenu);
}
...
function createWindow(): void {
  ...
  // Restore main app menu when this window gains focus (the popout browser
  // window installs its own menu, so we need to swap back).
  mainWindow.on("focus", () => {
    restoreAppMenu();
  });
...
app.whenReady().then(async () => {
  ...
  // Trust all certificates so the built-in browser can access sites with
  // self-signed or otherwise untrusted certs (e.g. internal dev servers).
  app.on(
    "certificate-error",
  ...
  protocol.handle("custom-pet", (request) => {
    ...
    const customPetsRoot = path.resolve(
      path.join(app.getPath("userData"), "custom-pets"),
    );
  ...
  // Load persisted browser extensions before creating the window so they are
  // available in the default session when the first <webview> is rendered.
  // Also hydrate HookServer with AI tab title settings.
  try {
    const state = await StateService.load();
    const extensionPaths = state?.settings?.extensionPaths ?? [];
    for (const extPath of extensionPaths) {
      try {
        await session.defaultSession.loadExtension(extPath);
      } catch (err) {
        console.warn(`[ForgePad] Failed to load extension: ${extPath}`, err);
      }
    }
    // Hydrate HookServer with initial AI tab title settings
    if (state?.settings) {
